...
description: >-
  ...
  "设备离线排查", "分析信号质量", "查看告警", "管理告警规则", "升级固件",
  "管理设备组", "创建远程隧道", "管理 DRC 模板", "边缘计算应用部署",
  "查看流量统计", "设备配置下发", "远程重启设备", "查看在线统计",
  "管理用户", "查看操作日志", "管理设备权限", "查看任务状态",
  or any device management, remote access, edge computing, system administration,
...
如果 `devicemanager` 命令不存在，读取安装指南并按步骤执行：

```
https://raw.githubusercontent.com/inhandnet/devicemanager-cli/main/INSTALL.md
```
...
devicemanager device import <file.xlsx> [--group <gid>] [--no-overwrite]  # 批量导入设备
...
```
devicemanager device alert-rule list                   # 规则列表
devicemanager device alert-rule get <id>               # 规则详情
devicemanager device alert-rule create --name <n> --alert-type <type> [--for-device-type ALL|DEVICE|DEVICE_GROUP] [--notify-types email,sms,webhook]  # 创建规则
devicemanager device alert-rule update <id> [--name <n>] [--notify-users uid,...] [--webhook-url <url>]  # 更新规则
devicemanager device alert-rule enable <id>            # 启用规则
devicemanager device alert-rule disable <id>           # 禁用规则
devicemanager device alert-rule delete <id>            # 删除规则
```
...
devicemanager task list --status running               # 按状态：running/waiting/failed/completed
devicemanager task list --type config-apply            # 按类型：config-apply/interactive-command/fetch-config/import-firmware/vpn-channel/vpn-link-order/token-cleanup/traffic-stats/idle-notice/remote-web
...
devicemanager drc devices add <id> [device-id]... [--group <gid>]  # 分配设备/分组
...
devicemanager edge control remove <device-id> <app-id>   # 卸载应用

# 设备边缘状态
devicemanager edge device <device-id>                  # 查看设备的引擎和应用状态
```
...
devicemanager firmware upload <file>                   # 上传固件文件
devicemanager firmware delete <firmware-id>            # 删除固件
...
```
# 用户
devicemanager system user list [--oid <org-id>]        # 用户列表（可指定组织）
devicemanager system user get <id>                     # 用户详情
devicemanager system user create --name <n> --email <e> [--role-id <rid>] [--external]  # 创建用户（邮件邀请设密码）
...
# 组织
devicemanager system org list [--name <n>] [--email <e>]  # 组织列表（支持过滤）
...
文档按型号组织在 [model-reference](https://github.com/inhandnet/model-reference) 仓库。
阅读路径：`index.md`（根）→ `<model>/index.md`（型号入口）→ 具体文档。

```
devicemanager docs list                                # 查看已收录型号
devicemanager docs list --model ER805                  # 查看型号文档目录
devicemanager docs search "VPN" --model ER805          # 在型号文档中搜索
devicemanager docs get ER805/troubleshooting/vpn.md    # 读取具体文档
devicemanager docs get config.md --model ER805         # 等价写法
```
...
- `--verbose` — 全局参数，详细级别（1-100，默认 100），仅对 GET（查询）请求生效
...
9. **不猜字段名**：不确定资源有哪些字段时，先用 `--limit 1` 取一条完整记录查看实际字段名（verbose 默认已是 100）
...
13. **设备文档优先**：涉及设备配置修改、故障排查时，先通过 `device list` 或 `device get` 查出设备型号（model 字段），然后用 `devicemanager docs search <关键词> --model <型号>` 查设备端文档，有匹配的就 `devicemanager docs get <path>` 读取后再回答。不要凭记忆猜配置步骤或排查方法
14. **`-q` 只属于 `api` 命令**：`-q` 是 `devicemanager api` 的 query param 简写，其他命令（device/firmware/system 等）不支持 `-q`。筛选设备用 `--name`/`--serial-number`/`--model`/`--online` 等具名 flag，不要用 `-q`
15. **时间范围左闭右开**：所有时间范围查询遵循 `start ≤ data < end`。查某天数据需要 `--start-time 当天 --end-time 次日`，如查 5 月 7 日：`--start-time 2026-05-07 --end-time 2026-05-08`
...
| 用户请求涉及                               | 读取                                   |
| ------------------------------------------ | -------------------------------------- |
| 设备离线/断网/连不上/重启/掉线排查         | `references/diagnostics.md`            |
...
| 告警规则/创建规则/启用禁用规则            | `references/commands/devicemanager_device_alert-rule.md` |
| 在线统计/在线率/掉线时长                  | `references/commands/devicemanager_device_online-stats.md` |
| 任务管理/任务状态/取消任务                | `references/commands/devicemanager_task.md` |
| 用户管理/权限管理/审计日志/组织信息        | `references/commands/devicemanager_system.md` |
| 上下线事件/掉线时间/注册日志              | `references/commands/devicemanager_device_online-events.md` |
| 边缘应用日志/edge app 报错               | `references/commands/devicemanager_edge_app_logs.md` |
| VPN/防火墙/DHCP/路由/端口映射/设备配置    | 先查设备型号，再 `devicemanager docs search <关键词> --model <型号>` |
| 设备端报错/无法上网/连接失败/配置不生效    | 先查设备型号，再 `devicemanager docs search <关键词> --model <型号>` |
